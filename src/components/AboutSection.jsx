...
import { Code, Download, Sparkles, Star, User } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { aboutData, personalInfo, socialLinks } from "@/data/portfolio";
...
export const AboutSection = () => {
  ...
  const { heading, subheading, profileImage, achievements, tabContent, techStack, features } = aboutData;
  ...
  const handleDownload = () => {
    ...
    link.href = personalInfo.resumeUrl; // Must be in public folder
      ...
      <div className="container mx-auto max-w-7xl relative">
        ...
        <div className="text-center mb-16 md:mb-20 px-2 sm:px-6">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-bold mb-4 sm:mb-6">
            <span className="bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent">{heading.split(' ')[0]}</span>
            <span className="block text-primary animate-pulse">{heading.split(' ').slice(1).join(' ')}</span>
          </h1>
              ...
              <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-background/50 rounded-xl border border-border">
                ...
                <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
                  {socialLinks.map((social, index) => (
                    <a key={index} href={social.url} className="p-2 bg-background rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all duration-300 hover:scale-110"><social.icon className="h-5 w-5" /></a>
                  ))}
                </div>
              </div>
            </div>
*/}
            <ProfileCard
              ...
              title={personalInfo.aboutRole}
              handle={socialLinks.find(s => s.label === "GitHub")?.url.split('/').pop() || "edderyouch"}
              status="Online"
              contactText="Contact Me"
              avatarUrl={aboutData.profileImage}
              showUserInfo={true}
              enableTilt={true}
