...
import { heroData, personalInfo } from "@/data/portfolio";
...
export const HeroSection = () => {
  ...
  const { codeSnippets, achievements, description, highlightedRole, greeting, availability } = heroData;
          ...
          <div className="flex-1 text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
            ...
            <motion.div className="mt-6 text-center lg:text-left" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              <div className="text-sm text-muted-foreground">
                🚀 <span className="text-primary font-semibold">{availability.split(' for ')[0]}</span> for {availability.split(' for ')[1] || ''}
              </div>
            </motion.div>
          </div>
                    ...
                    <div className="grid grid-cols-1 gap-1 h-full content-start">
                      {codeSnippets.map((line, index) => (
                        <div
                          key={index}
                          className={`
                            min-h-[20px] flex items-start justify-start text-start
